...
ON DUPLICATE KEY UPDATE `courseID` = VALUES(`courseID`), `title` = VALUES(`title`);
...
(1,	2),
(2,	1)
ON DUPLICATE KEY UPDATE `courseID` = VALUES(`courseID`), `studentID` = VALUES(`studentID`);
...
(1,	'Joe',	'Smith'),
(2,	'Sally',	'Smith')
ON DUPLICATE KEY UPDATE `studentID` = VALUES(`studentID`), `firstname` = VALUES(`firstname`), `lastname` = VALUES(`lastname`);
